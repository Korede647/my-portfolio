export const navItems = [
  { name: "About", link: "#about" },
  { name: "Projects", link: "#projects" },
  { name: "Experience", link: "#experience" },
  { name: "Contact", link: "#contact" },
];

export const myTechStack = [
  "TypeScript",
  "React",
  "Angular",
  "NodeJS",
  "Express",
  "JavaScript",
  "CSS",
  "HTML",
  "Git",
  "Java",
  "Python",
  "SpringBoot",
  "Dart",
  "Flutter",
  "System Design",
  "React Native",
  "Redis",
  "MongoDB",
  "PostgreSQL",
  "React",
  "Next.js",
];

export const projects = [
  {
    id: 1,
    title: "Fanimation",
    description:
      "Developed Fanimation, a platform designed to connect users with the endless different types of fans and their different categories. Basically, a fan blog with filters to assist users to find the kind of fan they're interested in.",
    className: "lg:col-span-3 md:col-span-6 md:row-span-4 lg:min-h-[60vh]",
    titleClassName: "justify-end",
    img: "https://i.pinimg.com/originals/be/f4/1a/bef41a7d5a877841bbf7d8f9f0d42f14.gif",
    github: "https://github.com/Korede647/Fanimation",
    link: "https://fanimation-project.vercel.app/",
    techs: ["JavaScript", "React", "CSS"],
  },
  {
    id: 2,
    title: "Netflix Clone",
    description:
      "A clone of Netflix's website",
    className: "lg:col-span-2 md:col-span-3 md:row-span-2",
    titleClassName: "justify-start",
    img: "https://i.pinimg.com/originals/84/f6/d1/84f6d14f1f88d34d3956150d19060d3a.gif",
    github: "https://github.com/Korede647/Netflix-Clone",
    link: "https://netflix-clone-one-nu-90.vercel.app/",
    techs: ["JavaScript", "HTML", "CSS", "GitHub"],
  },
  {
    id: 3,
    title: "Twitter Clone",
    description:
      "A clone of Twitter's website",
    className: "lg:col-span-2 md:col-span-3 md:row-span-2",
    titleClassName: "justify-center",
    img: "https://i.pinimg.com/originals/54/b5/24/54b52468335fd6eb935e330eb3197b25.gif",
    github: "https://github.com/Korede647/Twitter",
    link: "https://twitter-clone-blond-iota.vercel.app/",
    techs: ["JavaScript", "HTML", "CSS", "GitHub"],
  },
  {
    id: 4,
    title: "Lorem Quest: Level Up With CWT",
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Voluptatum, officiis, eum modi inventore natus velit voluptates laudantium nam.",
    className: "lg:col-span-2 md:col-span-3 md:row-span-1",
    titleClassName: "justify-start",
    img: "https://i.pinimg.com/736x/c7/de/01/c7de016c811fa5fae9d7120402f27645.jpg",
    github: "https://github.com/Korede647",
    link: "https://codewithty.dev",
    techs: ["JavaScript", "NodeJS", "Express", "React", "Heroku", "GitHub"],
  },
  {
    id: 5,
    title: "JS Lorem Animation : Making Ipsum Transitions",
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Voluptatum, officiis, eum modi inventore natus velit voluptates laudantium nam!",
    className: "md:col-span-3 md:row-span-2",
    titleClassName: "justify-center md:justify-start lg:justify-center",
    img: "https://i.pinimg.com/736x/09/2e/25/092e252b5562c7e5eb576215c6d9db8d.jpg",
    github: "https://github.com/Korede647",
    link: "https://codewithty.dev/about",
    techs: ["JavaScript", "CSS", "React", "Tailwind", "GitHub", "AWS"],
  },
  {
    id: 6,
    title: "Universe Lorem: Code Your Way Through Galaxies",
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Voluptatum, officiis, eum modi inventore natus velit voluptates laudantium nam.",
    className: "lg:col-span-2 md:col-span-3 md:row-span-1",
    img: "https://i.pinimg.com/originals/bf/c3/fb/bfc3fb764ff5f4d8d9ecb6da8544709c.gif",
    github: "https://github.com/codewithty",
    link: "https://codewithty.dev/contact",
    techs: ["JavaScript", "React", "NodeJS", "CSS", "NextJS", "GitHub"],
  },
];

export const workExperience = [
  {
    id: 1,
    company: "Aptech",
    title: "Full Stack Sorcerer",
    desc: "Simplified complex tech concepts into engaging lessons, mentoring students to build real-world projects and boosting overall class project completion rates by 25%.",
    className: "md:col-span-2",
    location: "Port-Harcourt, Nigeria",
    period: "8th Jan, 2024 - Currently",
    skills: [
      "HTML",
      "CSS",
      "JavaScript",
      "TypeScript",
      "React",
      "NodeJS",
      "Java",
      "SpringBoot",
    ],
  },
  {
    id: 2,
    company: "Aqqute Labs",
    title: "Backend Developer",
    desc: "Designed and implemented scalable database schemas, cutting query execution time by 40% and supporting growth from thousands to millions of records seamlessly.",
    className: "md:col-span-2",
    location: "Port-Harcourt, Nigeria",
    period: "Feb-2nd-2025 - Currently",
    skills: [
      "Figma",
      "NodeTS",
      "MongoDB"
    ],
  },
];

export const socialMedia = [
  {
    id: 1,
    img: "assets/git.svg",
    link: "https://github.com/Korede647",
  },
  {
    id: 2,
    img: "assets/linkedin.svg",
    link: "#",
  },
  {
    id: 3,
    img: "assets/insta.svg",
    link: "https://www.instagram.com/_sochi.fro/",
  }
];

export const skills = [
  "TypeScript",
  "React",
  "Angular",
  "NodeJS",
  "Express",
  "NextJS",
  "Tailwind",
  "JavaScript",
  "CSS",
  "HTML",
  "Git",
  "Java",
  "Python",
  "SpringBoot",
  "Dart",
  "Flutter",
  "System Design",
  "React Native",
  "Redis",
  "MongoDB",
  "PostgreSQL",
  "Figma"
];
